...
const setupTools = (server: McpServer, config: Record<string, unknown>) => {
  if (config.getGlobalNewsList) {
    server.registerTool(
  ...
  if (config.getGlobalReleasesList) {
    server.registerTool(
  ...
  if (config.getGlobalReleaseDetails) {
    server.registerTool(
